...
public:
    ...
    static Grep build_grep(std::string_view path, std::string_view pattern, unsigned int max_memory, unsigned int max_threads = 0);
...
private:
    ...
    /// @param max_memory - buffer used by queued chunks
    /// @param max_threads - number of threads to use
    explicit Grep(std::string_view path, std::string_view pattern, unsigned int max_memory, unsigned int max_threads = 0);
    ...
    void grep_chunk(const std::vector<char>& chunk, unsigned long offset, unsigned long chunk_count, std::shared_ptr<const std::string> file_name);
